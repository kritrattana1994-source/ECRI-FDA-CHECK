      ...
      <div className="glass-panel rounded-2xl p-6 bg-white/80 space-y-4">
        <div className="flex flex-wrap justify-between items-center gap-3 border-b border-sky-100 pb-3">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="ค้นหารหัส, ยี่ห้อ, รุ่น, หรือหัวข้อ..."
              value={searchKeyword}
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 text-slate-600 uppercase text-[10px] font-extrabold tracking-wider border-b border-slate-100">
              <tr>
                ...
                <th className="p-3">ยี่ห้อ / รุ่น / ชนิด</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loadingAlerts ? (
                ...
                filteredAlerts.map((item, index) => {
                  const isCertified = item.certifyStatus === 'จริง' || item.certifyStatus === 'รับรองแล้ว';
                  const isRejected = item.certifyStatus === 'เท็จ' || item.certifyStatus === 'ปฏิเสธ';

                  return (
                    <tr key={index} className="hover:bg-sky-50/40 transition">
                      <td className="p-3 font-mono font-bold text-slate-800">
                        <div>{item.deviceCode}</div>
                        <div className="text-[10px] text-slate-400">{item.assetId || '-'}</div>
                      </td>
                      <td className="p-3">
                        <div className="font-bold text-slate-800">{item.brand} {item.model}</div>
                        <div className="text-[10px] text-slate-500">{item.deviceType || item.thaiName || '-'}</div>
                      </td>
                      <td className="p-3">
                      ...
                      <td className="p-3 text-center">
                        <button
                          ...
                          className="px-2.5 py-1 bg-purple-50 hover:bg-purple-100 text-purple-700 border border-purple-200 rounded-lg text-[11px] font-bold transition cursor-pointer flex items-center gap-1 mx-auto"
                        >
                      ...
                      <td className="p-3 text-center">
                        <span className={`text-[10px] font-extrabold px-2.5 py-1 rounded-full ${
                          ...
                          {item.certifyStatus || 'รอตรวจสอบ'}
                        </span>
                        {item.certifier && (
                          <div className="text-[9px] text-slate-400 mt-0.5">โดย: {item.certifier}</div>
                        )}
                      </td>
                      <td className="p-3 text-center">
